...
{
    public partial class AtualizaQuartilUm : System.Web.UI.Page
    {
        [WebMethod]
        public static double temperaturaQuartilUm()
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("SELECT DISTINCT PERCENTILE_CONT(0.25) WITHIN GROUP(ORDER BY [valor]) OVER(PARTITION BY 1) from leitura", conn))
                {
