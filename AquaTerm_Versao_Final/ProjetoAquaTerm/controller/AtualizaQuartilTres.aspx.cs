...
{
    public partial class AtualizaQuartilTres : System.Web.UI.Page
    {
        [WebMethod]
        public static double temperaturaQuartilTres()
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("SELECT DISTINCT PERCENTILE_CONT(0.75) WITHIN GROUP(ORDER BY [valor]) OVER(PARTITION BY 1) from leitura", conn))
                {
